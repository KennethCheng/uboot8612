...
#if defined(CONFIG_SMDK6400) || defined(CONFIG_SMDK6410) || defined(CONFIG_SMDK6430) || defined(CONFIG_SQ8612EVB)
...
#define sq_hsmmc_readl(x)	readl((ELFIN_HSMMC_BASE + (HSMMC_CHANNEL * 0x100000)) + (x))
#define sq_hsmmc_readw(x)	readw((ELFIN_HSMMC_BASE + (HSMMC_CHANNEL * 0x100000)) + (x))
#define sq_hsmmc_readb(x)	readb((ELFIN_HSMMC_BASE + (HSMMC_CHANNEL * 0x100000)) + (x))

#define sq_hsmmc_writel(v,x)	writel((v),(ELFIN_HSMMC_BASE + (HSMMC_CHANNEL * 0x100000)) + (x))
#define sq_hsmmc_writew(v,x)	writew((v),(ELFIN_HSMMC_BASE + (HSMMC_CHANNEL * 0x100000)) + (x))
#define sq_hsmmc_writeb(v,x)	writeb((v),(ELFIN_HSMMC_BASE + (HSMMC_CHANNEL * 0x100000)) + (x))
...
static int wait_for_r_buf_ready (void)
{
	...
	while (!(sq_hsmmc_readw(HM_NORINTSTS) & 0x20)) {
		if (uLoop % 500000 == 0 && uLoop > 0) {
			return 0;
		}
		uLoop++;
...
static int wait_for_cmd_done (void)
{
	...
	for (i=0; i<0x20000000; i++) {
		n_int = sq_hsmmc_readw(HM_NORINTSTS);
		dbg("  HM_NORINTSTS: %04x\n", n_int);
		if (n_int & 0x8000) break;
		if (n_int & 0x0001) return 0;
	}

	e_int = sq_hsmmc_readw(HM_ERRINTSTS);
	sq_hsmmc_writew(e_int, HM_ERRINTSTS);
	sq_hsmmc_writew(n_int, HM_NORINTSTS);
...
{
	while (!(sq_hsmmc_readw(HM_NORINTSTS) & 0x2))
		return 1;
}

static void ClearCommandCompleteStatus(void)
{
	sq_hsmmc_writew(1 << 0, HM_NORINTSTS);
	while (sq_hsmmc_readw(HM_NORINTSTS) & 0x1) {
		sq_hsmmc_writew(1 << 0, HM_NORINTSTS);
	}
}
...
{
	sq_hsmmc_writew(sq_hsmmc_readw(HM_NORINTSTS) | (1 << 1), HM_NORINTSTS);
	while (sq_hsmmc_readw(HM_NORINTSTS) & 0x2) {
		sq_hsmmc_writew(sq_hsmmc_readw(HM_NORINTSTS) | (1 << 1), HM_NORINTSTS);
	}
}
...
{
	sq_hsmmc_writew(sq_hsmmc_readw(HM_NORINTSTS) | (1 << 5), HM_NORINTSTS);
	while (sq_hsmmc_readw(HM_NORINTSTS) & 0x20)
		sq_hsmmc_writew(sq_hsmmc_readw(HM_NORINTSTS) | (1 << 5), HM_NORINTSTS);
}
...
{
	sq_hsmmc_writew((sq_hsmmc_readw(HM_NORINTSTSEN) & 0xFEFF) | (temp << 8), HM_NORINTSTSEN);
}

void hsmmc_reset (void)
{
	sq_hsmmc_writeb(0x3, HM_SWRST);
}
...
{
	sq_hsmmc_writew((sq_hsmmc_readw(HM_TRNMOD) & ~(0xffff)) | (MultiBlk << 5)
...
{
	sq_hsmmc_writel(arg, HM_ARGUMENT);
}

static void set_blkcnt_register(ushort uBlkCnt)
{
	sq_hsmmc_writew(uBlkCnt, HM_BLKCNT);
}

static void SetSystemAddressReg(uint SysAddr)
{
	sq_hsmmc_writel(SysAddr, HM_SYSAD);
}

static void set_blksize_register(ushort uDmaBufBoundary, ushort uBlkSize)
{
	sq_hsmmc_writew((uDmaBufBoundary << 12) | (uBlkSize), HM_BLKSIZE);
}

static void ClearErrInterruptStatus(void)
{
	while (sq_hsmmc_readw(HM_NORINTSTS) & (0x1 << 15)) {
		sq_hsmmc_writew(sq_hsmmc_readw(HM_NORINTSTS), HM_NORINTSTS);
		sq_hsmmc_writew(sq_hsmmc_readw(HM_ERRINTSTS), HM_ERRINTSTS);
	}
}
...
static void InterruptEnable(ushort NormalIntEn, ushort ErrorIntEn)
{
	...
	sq_hsmmc_writew(NormalIntEn, HM_NORINTSTSEN);
	sq_hsmmc_writew(ErrorIntEn, HM_ERRINTSTSEN);
}
...
static void hsmmc_clock_onoff (int on)
{
	...
	if (on == 0) {
		reg16 = sq_hsmmc_readw(HM_CLKCON) & ~(0x1<<2);
		sq_hsmmc_writew(reg16, HM_CLKCON);
	} else {
		reg16 = sq_hsmmc_readw(HM_CLKCON);
		sq_hsmmc_writew(reg16 | (0x1<<2), HM_CLKCON);
...
static void set_clock (uint clksrc, uint div)
{
	...
	sq_hsmmc_writel(0xC0000100 | (clksrc << 4), HM_CONTROL2);	// feedback control off
	sq_hsmmc_writel(0x00000000, HM_CONTROL3);
#else
	sq_hsmmc_writel(0xC0004100 | (clksrc << 4), HM_CONTROL2);	// rx feedback control
	sq_hsmmc_writel(0x00008080, HM_CONTROL3); 			// Low clock: 00008080
	sq_hsmmc_writel(0x3 << 16, HM_CONTROL4);
#endif

	sq_hsmmc_writew(sq_hsmmc_readw(HM_CLKCON) & ~(0xff << 8), HM_CLKCON);

	/* SDCLK Value Setting + Internal Clock Enable */
	sq_hsmmc_writew(((div<<8) | 0x1), HM_CLKCON);
...
static int issue_command (ushort cmd, uint arg, uint data, uint flags)
{
	...
	for (i=0; i<0x1000000; i++) {
		if (!(sq_hsmmc_readl(HM_PRNSTS) & 0x1))
			break;
	}
	if (i == 0x1000000) {
		printf("@@@@@@1 rHM_PRNSTS: %08lx\n", sq_hsmmc_readl(HM_PRNSTS));
	}

	/* Check CommandInhibit_DAT */
	if (flags & MMC_RSP_BUSY) {
		for (i=0; i<0x1000000; i++) {
			if (!(sq_hsmmc_readl(HM_PRNSTS) & 0x2))
				break;
		}
		if (i == 0x1000000) {
	...
	if (!(sq_hsmmc_readw(HM_NORINTSTS) & 0x8000)) {
		return 1;
	} else {
		if (ocr_check == 1)
			return 0;
		else {
			printf("Command = %d, Error Stat = 0x%04x\n", (sq_hsmmc_readw(HM_CMDREG) >> 8), sq_hsmmc_readw(HM_ERRINTSTS));
			return 0;
		}
	}
	...
	if (!issue_command(MMC_SET_BLOCKLEN, 8, 0, MMC_RSP_R1))
		...
		if (!issue_command(MMC_APP_CMD, rca<<16, 0, MMC_RSP_R1))
			...
			if (!issue_command(SD_APP_SEND_SCR, 0, 1, MMC_RSP_R1))
				...
				uSCR1 = sq_hsmmc_readl(HM_BDATA);
				uSCR2 = sq_hsmmc_readl(HM_BDATA);
...
static int check_card_status(void)
{
	if (!issue_command(MMC_SEND_STATUS, rca<<16, 0, MMC_RSP_R1)) {
		...
		if (((sq_hsmmc_readl(HM_RSPREG0) >> 9) & 0xf) == 4) {
...
static void set_hostctl_speed (uchar mode)
{
	...
	reg8 = sq_hsmmc_readb(HM_HOSTCTL) & ~(0x1<<2);
	sq_hsmmc_writeb(reg8 | (mode<<2), HM_HOSTCTL);
}
...
{
	uint arg = 0;
	uchar reg = sq_hsmmc_readb(HM_HOSTCTL);
	uchar bitmode = 0;
	...
	sq_hsmmc_writeb(reg, HM_HOSTCTL);
	card_irq_enable(1);
	dbg(" transfer rHM_HOSTCTL(0x28) = 0x%02x\n", sq_hsmmc_readb(HM_HOSTCTL));
...
static void check_dma_int (void)
{
	...
	for (i = 0; i < 0x1000000; i++) {
		if (sq_hsmmc_readw(HM_NORINTSTS) & 0x0002) {
		dbg("Transfer Complete\n");
		HS_DMA_END = 1;
		sq_hsmmc_writew(sq_hsmmc_readw(HM_NORINTSTS) | 0x0002, HM_NORINTSTS);
		break;
		}
		if (sq_hsmmc_readw(HM_NORINTSTS) & 0x8000) {
			printf("error found: %04x\n", sq_hsmmc_readw(HM_ERRINTSTS));
			break;
		}
	}
}
...
static void display_card_info (void)
{
	...
	if (movi_hc) {
		...
		for (i=0; i<4; i++) {
			resp[i] = sq_hsmmc_readl(HM_RSPREG0+i*4);
			dbg("%08x\n", resp[i]);
		}
...
static void DataRead_ForCompare (int StartAddr)
{
	...
	sq_hsmmc_writew(sq_hsmmc_readw(HM_NORINTSIGEN) & ~(0xffff), HM_NORINTSIGEN);
	...
	for (j = 0; j < BlockNum_HSMMC; j++) {
		if (!wait_for_r_buf_ready())
			...
			ClearBufferReadReadyStatus();
		for (i = 0; i < 512 / 4; i++) {
			*Compare_buffer_HSMMC++ = sq_hsmmc_readl(HM_BDATA);
			CompareCnt_INT++;
		}
	}
	...
	dbg("\n\nHM_NORINTSTS = %x", sq_hsmmc_readw(HM_NORINTSTS));
}
...
int hsmmc_init (void)
{
	...
	sq_hsmmc_writeb(0xe, HM_TIMEOUTCON);
	set_hostctl_speed(NORMAL);

	InterruptEnable(0xff, 0xff);

	dbg("HM_NORINTSTS = %x\n", sq_hsmmc_readw(HM_NORINTSTS));
	...
	/* Manufacturer ID */
	card_mid = (sq_hsmmc_readl(HM_RSPREG3) >> 16) & 0xFF;

	dbg("Product Name : %c%c%c%c%c%c\n", ((sq_hsmmc_readl(HM_RSPREG2) >> 24) & 0xFF),
	       ((sq_hsmmc_readl(HM_RSPREG2) >> 16) & 0xFF), ((sq_hsmmc_readl(HM_RSPREG2) >> 8) & 0xFF), (sq_hsmmc_readl(HM_RSPREG2) & 0xFF),
	       ((sq_hsmmc_readl(HM_RSPREG1) >> 24) & 0xFF), ((sq_hsmmc_readl(HM_RSPREG1) >> 16) & 0xFF));
	...
	if (!mmc_card) {
		rca = (sq_hsmmc_readl(HM_RSPREG0) >> 16) & 0xFFFF;
		//printf("=>  rca=0x%08x\n", rca);
	}
	...
	if (mmc_card) {
		mmc_spec = (sq_hsmmc_readl(HM_RSPREG3) >> 18) & 0xF;
		dbg("mmc_spec=%d\n", mmc_spec);
	}
	...
	sq_hsmmc_writew(0xffff, HM_NORINTSTS);
...
void movi_write (uint addr, uint start_blk, uint blknum)
{
	...
	sq_hsmmc_writew((sq_hsmmc_readw(HM_NORINTSTSEN) & ~(0xffff)) |
	...
	sq_hsmmc_writew((sq_hsmmc_readw(HM_NORINTSIGEN) & ~(0xffff)) | TRANSFERCOMPLETE_SIG_INT_EN, HM_NORINTSIGEN);
	...
	sq_hsmmc_writew(sq_hsmmc_readw(HM_NORINTSTS) | (1 << 3), HM_NORINTSTS);
...
void movi_read (uint addr, uint start_blk, uint blknum)
{
	...
	sq_hsmmc_writew(sq_hsmmc_readw(HM_NORINTSTSEN) & ~(DMA_STS_INT_EN | BLOCKGAP_EVENT_STS_INT_EN), HM_NORINTSTSEN);
	sq_hsmmc_writew((HM_NORINTSIGEN & ~(0xffff)) | TRANSFERCOMPLETE_SIG_INT_EN, HM_NORINTSIGEN);
