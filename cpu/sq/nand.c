...
#define SQ_NAND_TYPE_UNKNOWN	0x0
#define SQ_NAND_TYPE_SLC	0x1
#define SQ_NAND_TYPE_MLC	0x2
#undef	SQ_NAND_DEBUG

/* Nand flash global values by jsgood */
int cur_ecc_mode = 0;
int nand_type = SQ_NAND_TYPE_UNKNOWN;

/* Nand flash oob definition for SLC 512b page size by jsgood */
static struct nand_ecclayout sq_nand_oob_16 = {
...
static struct nand_ecclayout sq_nand_oob_64 = {
	.useecc = MTD_NANDECC_AUTOPLACE,	/* Only for U-Boot */
	.eccbytes = 16,
...
static struct nand_ecclayout sq_nand_oob_mlc_64 = {
	.useecc = MTD_NANDECC_AUTOPLACE,	/* Only for U-Boot */
	.eccbytes = 32,
	.eccpos = {
 ...
 * Written by jsgood
 */
static void sq_nand_hwcontrol(struct mtd_info *mtd, int dat, unsigned int ctrl)
{
	unsigned int cur;
 ...
 * Written by jsgood
 */
static int sq_nand_device_ready(struct mtd_info *mtdinfo)
{
 ...
 */
static int sq_nand_scan_bbt(struct mtd_info *mtdinfo)
{
...
#if 0
/*
 * SQ Nand flash chip enable function
 * Written by jsgood
 */
static void sq_nand_ce_on(struct mtd_info *mtd)
{
	struct nand_chip *chip = mtd->priv;
...
/*
 * SQ Nand flash chip disable function
 * Written by jsgood
 */
static void sq_nand_ce_off(struct mtd_info *mtd)
{
	struct nand_chip *chip = mtd->priv;
 ...
 * Written by jsgood
 */
static void sq_nand_wait_enc(void)
{
 ...
 * Written by jsgood
 */
static void sq_nand_wait_dec(void)
{
 ...
 * Written by jsgood
 */
static void sq_nand_wait_ecc_busy(void)
{
 ...
 * Written by jsgood
 */
static void sq_nand_enable_hwecc(struct mtd_info *mtd, int mode)
{
 ...
 * Written by jsgood
 */
static int sq_nand_calculate_ecc(struct mtd_info *mtd, const u_char *dat, u_char *ecc_code)
{
 ...
 * Written by jsgood
 */
static int sq_nand_correct_data(struct mtd_info *mtd, u_char *dat, u_char *read_ecc, u_char *calc_ecc)
{
	int ret = -1;
	...
	if (nand_type == SQ_NAND_TYPE_SLC) {
		...
		case 1: /* 1 bit error (Correctable)
			...
			printk("sq-nand: 1 bit error detected at byte %ld, correcting from "
		...
		case 3: /* ECC area error */
			printk("sq-nand: ECC uncorrectable error detected\n");
			ret = -1;
			break;
		}
	} else {
		/* MLC: */
		sq_nand_wait_ecc_busy();
		...
		case 1: /* 1 bit error (Correctable) */
			printk("sq-nand: %d bit(s) error detected, corrected successfully\n", err_type);
...
#if defined(CONFIG_NAND_BL1_8BIT_ECC) && (defined(CONFIG_SQ8612))
/***************************************************************
 * jsgood: Temporary 8 Bit H/W ECC supports for BL1 (SQ8612 only)
 ***************************************************************/
static void sq_nand_wait_ecc_busy_8bit(void)
{
	while (readl(NF8ECCERR0) & NFESTAT0_ECCBUSY) {}
}

void sq_nand_enable_hwecc_8bit(struct mtd_info *mtd, int mode)
{
	u_long nfcont, nfconf;
...
int sq_nand_calculate_ecc_8bit(struct mtd_info *mtd, const u_char *dat, u_char *ecc_code)
{
...
int sq_nand_correct_data_8bit(struct mtd_info *mtd, u_char *dat, u_char *read_ecc, u_char *calc_ecc)
{
	int ret = -1;
	u_long nf8eccerr0, nf8eccerr1, nf8eccerr2, nfmlc8bitpt0, nfmlc8bitpt1;
	u_char err_type;

	sq_nand_wait_ecc_busy_8bit();
...
void sq_nand_write_page_8bit(struct mtd_info *mtd, struct nand_chip *chip,
				  const uint8_t *buf)
{
	int i, eccsize = 512;
	int eccbytes = 13;
	...
	for (i = 0; eccsteps; eccsteps--, i += eccbytes, p += eccsize) {
		sq_nand_enable_hwecc_8bit(mtd, NAND_ECC_WRITE);
		chip->write_buf(mtd, p, eccsize);
		sq_nand_calculate_ecc_8bit(mtd, p, &ecc_calc[i]);
	}
...
int sq_nand_read_page_8bit(struct mtd_info *mtd, struct nand_chip *chip,
				uint8_t *buf)
{
	...
	for (i = 0; eccsteps; eccsteps--, i += eccbytes, p += eccsize) {
		chip->cmdfunc(mtd, NAND_CMD_RNDOUT, col, -1);
		sq_nand_enable_hwecc_8bit(mtd, NAND_ECC_READ);
		chip->read_buf(mtd, p, eccsize);
		chip->write_buf(mtd, chip->oob_poi + (((mtd->writesize / eccsize) - eccsteps) * eccbytes), eccbytes);
		sq_nand_calculate_ecc_8bit(mtd, 0, 0);
		stat = sq_nand_correct_data_8bit(mtd, p, 0, 0);
...
static int sq_nand_write_oob_1bit(struct mtd_info *mtd, struct nand_chip *chip,
			      int page)
{
	uint8_t *ecc_calc = chip->buffers->ecccalc;
	int status = 0;
...
static int sq_nand_read_oob_1bit(struct mtd_info *mtd, struct nand_chip *chip,
			     int page, int sndcmd)
{
	uint8_t *ecc_calc = chip->buffers->ecccalc;
	int eccbytes = chip->ecc.bytes;
...
static void sq_nand_write_page_1bit(struct mtd_info *mtd, struct nand_chip *chip,
				  const uint8_t *buf)
{
...
static int sq_nand_read_page_1bit(struct mtd_info *mtd, struct nand_chip *chip,
				uint8_t *buf)
{
 ...
 * Written by jsgood
 */
static int sq_nand_read_page_4bit(struct mtd_info *mtd, struct nand_chip *chip,
				uint8_t *buf)
{
	int i, stat, eccsize = chip->ecc.size;
	int eccbytes = chip->ecc.bytes;
 ...
 * Written by jsgood
 */
static void sq_nand_write_page_4bit(struct mtd_info *mtd, struct nand_chip *chip,
				  const uint8_t *buf)
{
	int i, eccsize = chip->ecc.size;
	int eccbytes = chip->ecc.bytes;
...
void board_nand_init(struct nand_chip *nand)
{
	...
	nand->cmd_ctrl		= sq_nand_hwcontrol;
	nand->dev_ready		= sq_nand_device_ready;
	nand->scan_bbt		= sq_nand_scan_bbt;
	nand->options		= 0;
	...
	nand->ecc.hwctl		= sq_nand_enable_hwecc;
	nand->ecc.calculate	= sq_nand_calculate_ecc;
	nand->ecc.correct	= sq_nand_correct_data;
	
	sq_nand_hwcontrol(0, NAND_CMD_READID, NAND_NCE | NAND_CLE | NAND_CTRL_CHANGE);
	sq_nand_hwcontrol(0, 0x00, NAND_CTRL_CHANGE | NAND_NCE | NAND_ALE);
	sq_nand_hwcontrol(0, 0x00, NAND_NCE | NAND_ALE);
	sq_nand_hwcontrol(0, NAND_CMD_NONE, NAND_NCE | NAND_CTRL_CHANGE);
	sq_nand_device_ready(0);
	...
	if (!type->pagesize) {
		if (((nand->cellinfo >> 2) & 0x3) == 0 || 1) {
			nand_type = SQ_NAND_TYPE_SLC;
			nand->ecc.size = 512;
			nand->ecc.bytes	= 4;
			
			if ((1024 << (tmp & 0x3)) > 512) {
				nand->ecc.read_page = sq_nand_read_page_1bit;
				nand->ecc.write_page = sq_nand_write_page_1bit;
				nand->ecc.read_oob = sq_nand_read_oob_1bit;
				nand->ecc.write_oob = sq_nand_write_oob_1bit;
				nand->ecc.layout = &sq_nand_oob_64;
			} else {				
				nand->ecc.layout = &sq_nand_oob_16;
			}
		} else {
			nand_type = SQ_NAND_TYPE_MLC;
			nand->options |= NAND_NO_SUBPAGE_WRITE;	/* NOP = 1 if MLC */
			nand->ecc.read_page = sq_nand_read_page_4bit;
			nand->ecc.write_page = sq_nand_write_page_4bit;
			nand->ecc.size = 512;
			nand->ecc.bytes = 8;	/* really 7 bytes */
			nand->ecc.layout = &sq_nand_oob_mlc_64;
		}
	} else {
		nand_type = SQ_NAND_TYPE_SLC;
		nand->ecc.size = 512;
