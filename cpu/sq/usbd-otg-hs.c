/*
 * cpu/sq/usbd-otg-hs.c
 *
 *  $Id: usbd-otg-hs.c,v 1.3 2008/07/10 08:19:19 boyko Exp $
 *
 * (C) Copyright 2007
 * Kenneth Cheng, SQ Erectronics, KennethCheng@sq.com.tw
 *	- only support for SQ8612
 *
...
/* SQ_OTG_GINTSTS/SQ_OTG_GINTMSK core interrupt register */
...
/* SQ_OTG_DCTL device control register */
...
/* SQ_OTG_DAINT device all endpoint interrupt register */
...
/* SQ_OTG_DIEPCTL0/SQ_OTG_DOEPCTL0 */
...
void sq_usb_init_phy(void)
{
	writel(0x0, SQ_OTG_PHYPWR);
	writel(0x00, SQ_OTG_PHYCTRL);
	writel(0x1, SQ_OTG_RSTCON);
	udelay(10);
	writel(0x0, SQ_OTG_RSTCON);
	udelay(10);
}

void sq_usb_core_soft_reset(void)
{
	u32 tmp;

	writel(CORE_SOFT_RESET, SQ_OTG_GRSTCTL);

	do
	{
		tmp = readl(SQ_OTG_GRSTCTL);
	}while(!(tmp & AHB_MASTER_IDLE));

}

void sq_usb_wait_cable_insert(void)
{
	u32 tmp;
	int ucFirst=1;
...
void sq_usb_check_current_mode(u8 *pucMode)
{
	u32 tmp;

	tmp = readl(SQ_OTG_GINTSTS);
	*pucMode = tmp & 0x1;
}

void sq_usb_set_soft_disconnect(void)
{
	u32 tmp;

	tmp = readl(SQ_OTG_DCTL);
	tmp |= SOFT_DISCONNECT;
	writel(tmp, SQ_OTG_DCTL);
}

void sq_usb_clear_soft_disconnect(void)
{
	u32 tmp;
...
void sq_usb_init_device(void)
{
	writel(1<<18|otg.speed<<0, SQ_OTG_DCFG); /* [][1: full speed(30Mhz) 0:high speed]*/

	writel(INT_RESUME|INT_OUT_EP|INT_IN_EP|INT_ENUMDONE|
		INT_RESET|INT_SUSPEND|INT_RX_FIFO_NOT_EMPTY,
		SQ_OTG_GINTMSK);	/*gint unmask */
}

int sq_usbctl_init(void)
{
	u8 ucMode;
	...
	otg.ep0_substate = 0;
	sq_usb_init_phy();
	sq_usb_core_soft_reset();
	sq_usb_wait_cable_insert();
	sq_usb_init_core();
	sq_usb_check_current_mode(&ucMode);

	if (ucMode == INT_DEV_MODE) {
		sq_usb_set_soft_disconnect();
		udelay(10);
		sq_usb_clear_soft_disconnect();
		sq_usb_init_device();
		return 0;
	} else {
...
int sq_usbc_activate (void)
{
	/* dont used in usb high speed, but used in common file cmd_usbd.c  */
	return 0;
}

int sq_usb_stop (void)
{
	/* dont used in usb high speed, but used in common file cmd_usbd.c  */
	return 0;
}

void sq_usb_print_pkt(u8 *pt, u8 count)
{
	int i;
	printf("[sq_usb_print_pkt:");
...
void sq_usb_verify_checksum(void)
{
	u8 *cs_start, *cs_end;
	u16 dnCS;
	u16 checkSum;
...
void sq_usb_set_inep_xfersize(EP_TYPE type, u32 pktcnt, u32 xfersize)
{
	if(type == EP_TYPE_CONTROL)
	{
		writel((pktcnt<<19)|(xfersize<<0), SQ_OTG_DIEPTSIZ0);
	}
	else if(type == EP_TYPE_BULK)
	{
		writel((1<<29)|(pktcnt<<19)|(xfersize<<0), SQ_OTG_DIEPTSIZ_IN);
	}
}

void sq_usb_set_outep_xfersize(EP_TYPE type, u32 pktcnt, u32 xfersize)
{
	if(type == EP_TYPE_CONTROL)
	{
		writel((1<<29)|(pktcnt<<19)|(xfersize<<0), SQ_OTG_DOEPTSIZ0);
	}
	else if(type == EP_TYPE_BULK)
	{
		writel((pktcnt<<19)|(xfersize<<0), SQ_OTG_DOEPTSIZ_OUT);
	}
}

void sq_usb_write_ep0_fifo(u8 *buf, int num)
{
	int i;
	u32 Wr_Data=0;

	DBG_SETUP1("[sq_usb_write_ep0_fifo:");

	for(i=0;i<num;i+=4)
	{
		Wr_Data = ((*(buf+3))<<24)|((*(buf+2))<<16)|((*(buf+1))<<8)|*buf;
		DBG_SETUP2(" 0x%08x,", Wr_Data);
		writel(Wr_Data, SQ_OTG_EP0_FIFO);
		buf += 4;
	}
...
void sq_usb_write_in_fifo(u8 *buf, int num)
{
	int i;
	u32 data=0;
	...
	for(i=0;i<num;i+=4)
	{
		...
		writel(data, SQ_OTG_IN_FIFO);
		buf += 4;
	}
}

void sq_usb_read_out_fifo(u8 *buf, int num)
{
	int i;
	u32 data;
...
void sq_usb_clear_feature(void)
{
...
void sq_usb_set_feature(void)
{
	...
	case TEST_MODE:
		...
		case TEST_PACKET:
			...
			writel(EPEN_CNAK_EP0_64, SQ_OTG_DIEPCTL0);
			sq_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, TEST_PKT_SIZE);
			sq_usb_write_ep0_fifo(test_pkt, TEST_PKT_SIZE);
			tmp = readl(SQ_OTG_DCTL);
			tmp = (tmp & ~(TEST_CONTROL_FIELD)) | (TEST_PACKET_MODE);
			writel(tmp, SQ_OTG_DCTL);
			DBG_SETUP1 ("SQ_OTG_DCTL=0x%08x\n", tmp);
			break;
		}
		break;
...
void sq_usb_ep0_int_hndlr(void)
{
	u16 i;
	u32 buf[2]={0x0000, };
	u16 addr;
	...
	if (otg.ep0_state == EP0_STATE_INIT) {
...
#ifdef USB_OTG_DEBUG_SETUP
		sq_usb_print_pkt((u8 *)&otg.dev_req, 8);
#endif
		...
		case STANDARD_SET_ADDRESS:
			...
			writel(1<<18|addr<<4|otg.speed<<0, SQ_OTG_DCFG);
			DBG_SETUP1("SQ_OTG_DCFG : %x, STANDARD_SET_ADDRESS : %d\n",
					readl(SQ_OTG_DCFG), addr);
			otg.ep0_state = EP0_STATE_INIT;
			break;
		...
		case STANDARD_GET_CONFIGURATION:
			...
			sq_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 1);

			/*ep0 enable, clear nak, next ep0, 8byte */
			writel(EPEN_CNAK_EP0_8, SQ_OTG_DIEPCTL0);
			writel(config_value, SQ_OTG_EP0_FIFO);
			otg.ep0_state = EP0_STATE_INIT;
			break;
	...
	sq_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, otg.ctrl_max_pktsize);

	if(otg.speed == USB_HIGH) {
		/*clear nak, next ep0, 64byte */
		writel(((1<<26)|(CONTROL_EP<<11)|(0<<0)), SQ_OTG_DIEPCTL0);
	}
	else {
		/*clear nak, next ep0, 8byte */
		writel(((1<<26)|(CONTROL_EP<<11)|(3<<0)), SQ_OTG_DIEPCTL0);
	}

}

void sq_usb_set_otherspeed_conf_desc(u32 length)
{
	...
	if (otg.speed == USB_HIGH)
	{
	    if (length ==9)
	    {
		sq_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 9);
			writel(EPEN_CNAK_EP0_64, SQ_OTG_DIEPCTL0);
			sq_usb_write_ep0_fifo(((u8 *)&config_full)+0, 9);
		}
	    else if(length ==32)
		{
		sq_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 32);
			writel(EPEN_CNAK_EP0_64, SQ_OTG_DIEPCTL0);
			sq_usb_write_ep0_fifo(((u8 *)&config_full_total)+0, 32);
	...
	else
	{
	    if (length ==9)
	    {
		sq_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 8);
			writel(EPEN_CNAK_EP0_8, SQ_OTG_DIEPCTL0);
			sq_usb_write_ep0_fifo(((u8 *)&config_high)+0, 8);
		}
	    else if(length ==32)
		{
		sq_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 8);
			writel(EPEN_CNAK_EP0_64, SQ_OTG_DIEPCTL0);
			sq_usb_write_ep0_fifo(((u8 *)&config_high_total)+0, 8);
	    }
		otg.ep0_state = EP0_STATE_GD_OTHER_SPEED_HIGH_1;
	}
}

void sq_usb_transfer_ep0(void)
{
	...
	case EP0_STATE_INIT:
		sq_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 0);
	...
	case EP0_STATE_GD_DEV_0:
		...
		if (otg.speed == USB_HIGH) {
			...
			if (otg.req_length < DEVICE_DESC_SIZE) {
				sq_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, otg.req_length);
				sq_usb_write_ep0_fifo(((u8 *)&(otg.desc.dev))+0, otg.req_length);
			} else {
				sq_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, DEVICE_DESC_SIZE);
				sq_usb_write_ep0_fifo(((u8 *)&(otg.desc.dev))+0, DEVICE_DESC_SIZE);
			}
			otg.ep0_state = EP0_STATE_INIT;
		} else {
			...
			if(otg.req_length<FS_CTRL_PKT_SIZE) {
				...
				sq_usb_write_ep0_fifo(((u8 *)&(otg.desc.dev))+0, FS_CTRL_PKT_SIZE);
	...
	case EP0_STATE_GD_DEV_1:
		...
		if(otg.req_length<(2*FS_CTRL_PKT_SIZE)) {
			sq_usb_write_ep0_fifo(((u8 *)&(otg.desc.dev))+FS_CTRL_PKT_SIZE,
	...
	case EP0_STATE_GD_DEV_2:
		...
		writel(EPEN_CNAK_EP0_8, SQ_OTG_DIEPCTL0);
		if(otg.req_length<DEVICE_DESC_SIZE) {
			sq_usb_write_ep0_fifo(((u8 *)&(otg.desc.dev))+(2*FS_CTRL_PKT_SIZE),
	...
	case EP0_STATE_GD_CFG_0:
		...
		if (otg.speed == USB_HIGH)
		{
			...
			{
				sq_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, otg.req_length);
				sq_usb_write_ep0_fifo(((u8 *)&(otg.desc.config))+0, otg.req_length);
			}
			else
			{
				sq_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, CONFIG_DESC_TOTAL_SIZE);
				sq_usb_write_ep0_fifo(((u8 *)&(otg.desc.config))+0, CONFIG_DESC_TOTAL_SIZE);
			}
			otg.ep0_state = EP0_STATE_INIT;
		}
		else
		{
			...
			{
				sq_usb_write_ep0_fifo(((u8 *)&(otg.desc.config))+0, FS_CTRL_PKT_SIZE);
	...
	case EP0_STATE_GD_CFG_1:
		...
		if(otg.req_length<(2*FS_CTRL_PKT_SIZE))
		{
			sq_usb_write_ep0_fifo(((u8 *)&(otg.desc.config))+FS_CTRL_PKT_SIZE,
						(otg.req_length-FS_CTRL_PKT_SIZE));
			otg.ep0_state = EP0_STATE_INIT;
		}
		else
	...
	case EP0_STATE_GD_CFG_2:
		...
		{
			sq_usb_write_ep0_fifo(((u8 *)&(otg.desc.config))+(2*FS_CTRL_PKT_SIZE),
						(otg.req_length-2*FS_CTRL_PKT_SIZE));
			otg.ep0_state = EP0_STATE_INIT;
		}
		else
	...
	case EP0_STATE_GD_CFG_3:
		...
		{
			sq_usb_write_ep0_fifo(((u8 *)&(otg.desc.config))+(3*FS_CTRL_PKT_SIZE),
	...
	case EP0_STATE_GD_DEV_QUALIFIER:	/*only supported in USB 2.0*/
		...
		{
			sq_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, otg.req_length);
			sq_usb_write_ep0_fifo((u8 *)qualifier_desc+0, otg.req_length);
		}
		else
		{
			sq_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 10);
			sq_usb_write_ep0_fifo((u8 *)qualifier_desc+0, 10);
		}
		otg.ep0_state = EP0_STATE_INIT;
		break;
	...
	case EP0_STATE_GD_OTHER_SPEED:
			...
			sq_usb_set_otherspeed_conf_desc(otg.req_length);
			break;
	...
	case EP0_STATE_GD_OTHER_SPEED_HIGH_1:
		...
		if(otg.req_length==9)
		{
			sq_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 1);
				writel(EPEN_CNAK_EP0_8, SQ_OTG_DIEPCTL0);
				sq_usb_write_ep0_fifo(((u8 *)&config_high)+8, 1);
				otg.ep0_state = EP0_STATE_INIT;
		}
		else
		{
			sq_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 8);
				writel(EPEN_CNAK_EP0_8, SQ_OTG_DIEPCTL0);
				sq_usb_write_ep0_fifo(((u8 *)&config_high)+8, 8);
	...
	case EP0_STATE_GD_OTHER_SPEED_HIGH_2:
		...
		sq_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 8);
			writel(EPEN_CNAK_EP0_8, SQ_OTG_DIEPCTL0);
			sq_usb_write_ep0_fifo(((u8 *)&config_high)+16, 8);
	...
	case EP0_STATE_GD_OTHER_SPEED_HIGH_3:
		...
		sq_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 8);
			writel(EPEN_CNAK_EP0_8, SQ_OTG_DIEPCTL0);
			sq_usb_write_ep0_fifo(((u8 *)&config_high)+24, 8);
			otg.ep0_state = EP0_STATE_INIT;
			break;
	...
	case EP0_STATE_GD_CFG_ONLY_0:
		...
		if (otg.speed == USB_HIGH)
		{
			...
			{
				sq_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, CONFIG_DESC_SIZE);
				writel(EPEN_CNAK_EP0_64, SQ_OTG_DIEPCTL0);
				sq_usb_write_ep0_fifo(((u8 *)&(otg.desc.config))+0,
							CONFIG_DESC_SIZE);
			}
			otg.ep0_state = EP0_STATE_INIT;
		}
	...
	case EP0_STATE_GD_IF_ONLY_0:
		...
		if (otg.speed == USB_HIGH)
		{
			...
			if(otg.req_length<INTERFACE_DESC_SIZE)
			{
				sq_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, otg.req_length);
				writel(EPEN_CNAK_EP0_64, SQ_OTG_DIEPCTL0);
				sq_usb_write_ep0_fifo(((u8 *)&(otg.desc.intf))+0, otg.req_length);
			}
			else
			{
				sq_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, INTERFACE_DESC_SIZE);
				writel(EPEN_CNAK_EP0_64, SQ_OTG_DIEPCTL0);
				sq_usb_write_ep0_fifo(((u8 *)&(otg.desc.intf))+0, INTERFACE_DESC_SIZE);
			}
			otg.ep0_state = EP0_STATE_INIT;
		}
		else
		{
			...
			{
				sq_usb_write_ep0_fifo(((u8 *)&(otg.desc.intf))+0, FS_CTRL_PKT_SIZE);
	...
	case EP0_STATE_GD_IF_ONLY_1:
		...
		writel(EPEN_CNAK_EP0_8, SQ_OTG_DIEPCTL0);
		sq_usb_write_ep0_fifo(((u8 *)&(otg.desc.intf))+FS_CTRL_PKT_SIZE,
	...
	case EP0_STATE_GD_EP0_ONLY_0:
		...
		writel(EPEN_CNAK_EP0_8, SQ_OTG_DIEPCTL0);
		sq_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, ENDPOINT_DESC_SIZE);
		sq_usb_write_ep0_fifo(((u8 *)&(otg.desc.ep1))+0, ENDPOINT_DESC_SIZE);
		otg.ep0_state = EP0_STATE_INIT;
		break;
	...
	case EP0_STATE_GD_EP1_ONLY_0:
		...
		sq_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, ENDPOINT_DESC_SIZE);
		writel(EPEN_CNAK_EP0_8, SQ_OTG_DIEPCTL0);
		sq_usb_write_ep0_fifo(((u8 *)&(otg.desc.ep2))+0, ENDPOINT_DESC_SIZE);
		otg.ep0_state = EP0_STATE_INIT;
		break;
	...
	case EP0_STATE_GD_STR_I0:
		...
		sq_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, STRING_DESC0_SIZE);
		writel(EPEN_CNAK_EP0_8, SQ_OTG_DIEPCTL0);
		sq_usb_write_ep0_fifo((u8 *)string_desc0, STRING_DESC0_SIZE);
		otg.ep0_state = EP0_STATE_INIT;
		break;

	case EP0_STATE_GD_STR_I1:
		DBG_SETUP1("EP0_STATE_GD_STR_I1 %d\n", otg.ep0_substate);
		sq_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, sizeof(string_desc1));
		if ((otg.ep0_substate*otg.ctrl_max_pktsize+otg.ctrl_max_pktsize)
			...
			if (otg.speed == USB_HIGH)
				...
				writel(EPEN_CNAK_EP0_8, SQ_OTG_DIEPCTL0);
			sq_usb_write_ep0_fifo((u8 *)string_desc1+(otg.ep0_substate*otg.ctrl_max_pktsize),
	...
	case EP0_STATE_GD_STR_I2:
		sq_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, sizeof(string_desc2));
		if ((otg.ep0_substate*otg.ctrl_max_pktsize+otg.ctrl_max_pktsize)
			< sizeof(string_desc2)) {
			...
			if (otg.speed == USB_HIGH)
				...
				writel(EPEN_CNAK_EP0_8, SQ_OTG_DIEPCTL0);
			sq_usb_write_ep0_fifo((u8 *)string_desc2+(otg.ep0_substate*otg.ctrl_max_pktsize),
						otg.ctrl_max_pktsize);
			otg.ep0_state = EP0_STATE_GD_STR_I2;
			otg.ep0_substate++;
		} else {
	...
	case EP0_STATE_INTERFACE_GET:
		...
		sq_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 1);
		writel(EPEN_CNAK_EP0_8, SQ_OTG_DIEPCTL0);
		sq_usb_write_ep0_fifo((u8 *)&get_intf+0, 1);
		otg.ep0_state = EP0_STATE_INIT;
		break;
	...
	case EP0_STATE_GET_STATUS0:
		...
		sq_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 1);
		writel(EPEN_CNAK_EP0_8, SQ_OTG_DIEPCTL0);
		sq_usb_write_ep0_fifo((u8 *)&get_status+0, 1);
		otg.ep0_state = EP0_STATE_INIT;
		break;
	...
	case EP0_STATE_GET_STATUS1:
		...
		sq_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 1);
		writel(EPEN_CNAK_EP0_8, SQ_OTG_DIEPCTL0);
		sq_usb_write_ep0_fifo((u8 *)&get_status+1, 1);
		otg.ep0_state = EP0_STATE_INIT;
		break;
	...
	case EP0_STATE_GET_STATUS2:
		...
		sq_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 1);
		writel(EPEN_CNAK_EP0_8, SQ_OTG_DIEPCTL0);
		sq_usb_write_ep0_fifo((u8 *)&get_status+2, 1);
		otg.ep0_state = EP0_STATE_INIT;
		break;
	...
	case EP0_STATE_GET_STATUS3:
		...
		sq_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 1);
		writel(EPEN_CNAK_EP0_8, SQ_OTG_DIEPCTL0);
		sq_usb_write_ep0_fifo((u8 *)&get_status+3, 1);
		otg.ep0_state = EP0_STATE_INIT;
		break;
	...
	case EP0_STATE_GET_STATUS4:
		...
		sq_usb_set_inep_xfersize(EP_TYPE_CONTROL, 1, 1);
		writel(EPEN_CNAK_EP0_8, SQ_OTG_DIEPCTL0);
		sq_usb_write_ep0_fifo((u8 *)&get_status+4, 1);
		otg.ep0_state = EP0_STATE_INIT;
		break;
...
void sq_usb_int_bulkin(void)
{
	...
	if (remain_cnt > otg.bulkin_max_pktsize) {
		sq_usb_set_inep_xfersize(EP_TYPE_BULK, 1, otg.bulkin_max_pktsize);

		/*ep3 enable, clear nak, bulk, usb active, next ep3, max pkt 64*/
		writel(1u<<31|1<<26|2<<18|1<<15|otg.bulkin_max_pktsize<<0,
			SQ_OTG_DIEPCTL_IN);

		sq_usb_write_in_fifo(bulkin_buf, otg.bulkin_max_pktsize);

		otg.up_ptr += otg.bulkin_max_pktsize;

	} else if(remain_cnt > 0) {
		sq_usb_set_inep_xfersize(EP_TYPE_BULK, 1, remain_cnt);

		/*ep3 enable, clear nak, bulk, usb active, next ep3, max pkt 64*/
		writel(1u<<31|1<<26|2<<18|1<<15|otg.bulkin_max_pktsize<<0,
			SQ_OTG_DIEPCTL_IN);

		sq_usb_write_in_fifo(bulkin_buf, remain_cnt);

		otg.up_ptr += remain_cnt;

	} else { /*remain_cnt = 0*/
		writel((DEPCTL_SNAK|DEPCTL_BULK_TYPE), SQ_OTG_DIEPCTL_IN);
	}
}

void sq_usb_upload_start(void)
{
	u8 tmp_buf[12];
	u32 check;

	sq_usb_read_out_fifo((u8 *)tmp_buf, 10);
	...
	if (check==0x1) {
		...
		if (otg.op_mode == USB_CPU) {
			if (otg.up_size > otg.bulkin_max_pktsize) {
				sq_usb_set_inep_xfersize(EP_TYPE_BULK, 1,
			...
			writel(MODE_DMA|BURST_INCR4|GBL_INT_UNMASK,
				SQ_OTG_GAHBCFG);
			writel(INT_RESUME|INT_OUT_EP|INT_IN_EP| INT_ENUMDONE|
				INT_RESET|INT_SUSPEND, SQ_OTG_GINTMSK);

			writel((u32)otg.up_ptr, SQ_OTG_DIEPDMA_IN);
			...
			if (pktcnt > 1023) {
				sq_usb_set_inep_xfersize(EP_TYPE_BULK, 1023,
					(otg.bulkin_max_pktsize*1023));
			} else {
				sq_usb_set_inep_xfersize(EP_TYPE_BULK, pktcnt,
					otg.up_size);
			}
...
void sq_usb_download_start(u32 fifo_cnt_byte)
{
	u8 tmp_buf[8];

	sq_usb_read_out_fifo((u8 *)tmp_buf, 8);
	DBG_BULK1("downloadFileSize==0, 1'st BYTE_READ_CNT_REG : %x\n",
		fifo_cnt_byte);

	otg.dn_addr=sq_usbd_dn_addr;
	otg.dn_filesize=
		*((u8 *)(tmp_buf+4))+
		(*((u8 *)(tmp_buf+5))<<8)+
		(*((u8 *)(tmp_buf+6))<<16)+
	...
	sq_usb_read_out_fifo((u8 *)otg.dn_ptr, fifo_cnt_byte-8);
	otg.dn_ptr += fifo_cnt_byte-8;

	if (otg.op_mode == USB_CPU) {
		sq_usb_set_outep_xfersize(EP_TYPE_BULK, 1,
		...
		writel(INT_RESUME|INT_OUT_EP|INT_IN_EP|INT_ENUMDONE|
			INT_RESET|INT_SUSPEND, SQ_OTG_GINTMSK); /*gint unmask */
		writel(MODE_DMA|BURST_INCR4|GBL_INT_UNMASK,
			SQ_OTG_GAHBCFG);
		writel((u32)otg.dn_ptr, SQ_OTG_DOEPDMA_OUT);
		...
		if (pkt_cnt > 1023) {
			sq_usb_set_outep_xfersize(EP_TYPE_BULK, 1023,
				(otg.bulkout_max_pktsize*1023));
		} else {
			sq_usb_set_outep_xfersize(EP_TYPE_BULK, pkt_cnt,
...
void sq_usb_download_continue(u32 fifo_cnt_byte)
{
	if (otg.op_mode == USB_CPU) {
		sq_usb_read_out_fifo((u8 *)otg.dn_ptr, fifo_cnt_byte);
		otg.dn_ptr += fifo_cnt_byte;
		DBG_BULK1("downloadFileSize!=0, 2nd BYTE_READ_CNT_REG = 0x%x, m_pDownPt = 0x%x\n",
				fifo_cnt_byte, otg.dn_ptr);

		sq_usb_set_outep_xfersize(EP_TYPE_BULK, 1, otg.bulkout_max_pktsize);
...
void sq_usb_int_bulkout(u32 fifo_cnt_byte)
{
	...
	if (otg.dn_filesize==0) {
		if (fifo_cnt_byte == 10) {
			...
			sq_usb_download_start(fifo_cnt_byte);
		}
	} else {
		sq_usb_download_continue(fifo_cnt_byte);
	}
}

void sq_usb_dma_in_done(void)
{
	s32 remain_cnt;

	DBG_BULK0("DMA IN : Transfer Done\n");

	otg.up_ptr = (u8 *)readl(SQ_OTG_DIEPDMA_IN);
	remain_cnt = otg.up_size- ((u32)otg.up_ptr - otg.up_addr);
...
void sq_usb_dma_out_done(void)
{
	s32 remain_cnt;

	DBG_BULK1("DMA OUT : Transfer Done\n");
	otg.dn_ptr = (u8 *)readl(SQ_OTG_DOEPDMA_OUT);
	...
	if (remain_cnt>0) {
		...
		if (pktcnt> 1023) {
			...
			sq_usb_set_outep_xfersize(EP_TYPE_BULK, pktcnt,
				remain_cnt);
		}
...
void sq_usb_set_all_outep_nak(void)
{
	u8 i;
	u32 tmp;
	...
	for(i=0;i<16;i++)
	{
		tmp = readl(SQ_OTG_DOEPCTL0+0x20*i);
		tmp |= DEPCTL_SNAK;
		writel(tmp, SQ_OTG_DOEPCTL0+0x20*i);
	}
}

void sq_usb_clear_all_outep_nak(void)
{
	u8 i;
	u32 tmp;
...
void sq_usb_set_max_pktsize(USB_SPEED speed)
{
	if (speed == USB_HIGH)
	{
		otg.speed = USB_HIGH;
...
void sq_usb_set_endpoint(void)
{
	/* Unmask SQ_OTG_DAINT source */
	writel(0xff, SQ_OTG_DIEPINT0);
	writel(0xff, SQ_OTG_DOEPINT0);
	writel(0xff, SQ_OTG_DIEPINT_IN);
	writel(0xff, SQ_OTG_DOEPINT_OUT);
	...
	if(otg.speed == USB_HIGH)
	{
		...
		writel((1u<<31)|(1<<26)|(0<<0), SQ_OTG_DOEPCTL0);
	}
	else
	{
		/*MPS:8bytes */
		writel(((1<<26)|(CONTROL_EP<<11)|(3<<0)), SQ_OTG_DIEPCTL0);
		/*ep0 enable, clear nak */
		writel((1u<<31)|(1<<26)|(3<<0), SQ_OTG_DOEPCTL0);
	}
}

void sq_usb_set_descriptors(void)
{
	/* Standard device descriptor */
...
void sq_usb_check_speed(USB_SPEED *speed)
{
	u32 status;

	status = readl(SQ_OTG_DSTS); /* System status read */

	*speed = (USB_SPEED)((status&0x6) >>1);
}

void sq_usb_clear_dnfile_info(void)
{
	otg.dn_addr = 0;
	otg.dn_filesize = 0;
	otg.dn_ptr = 0;
...
void sq_usb_clear_upfile_info(void)
{
	otg.up_addr= 0;
	otg.up_size= 0;
	otg.up_ptr = 0;
...
void sq_usb_set_opmode(USB_OPMODE mode)
{
	otg.op_mode = mode;
	...
	writel(MODE_SLAVE|BURST_SINGLE|GBL_INT_UNMASK, SQ_OTG_GAHBCFG);

	sq_usb_set_outep_xfersize(EP_TYPE_BULK, 1, otg.bulkout_max_pktsize);
	sq_usb_set_inep_xfersize(EP_TYPE_BULK, 1, 0);
...
void sq_usb_reset(void)
{
	sq_usb_set_all_outep_nak();
	...
	/* Rx FIFO Size */
	writel(RX_FIFO_SIZE, SQ_OTG_GRXFSIZ);

	/* Non Periodic Tx FIFO Size */
	writel(NPTX_FIFO_SIZE<<16| NPTX_FIFO_START_ADDR<<0, SQ_OTG_GNPTXFSIZ);

	sq_usb_clear_all_outep_nak();

	/*clear device address */
	writel(readl(SQ_OTG_DCFG)&~(0x7f<<4), SQ_OTG_DCFG);

	if(SUSPEND_RESUME_ON) {
		writel(readl(SQ_OTG_PCGCCTRL)&~(1<<0), SQ_OTG_PCGCCTRL);
	}
}
int sq_usb_set_init(void)
{
	u32 status;
	...
	if (((status&0x6) >>1) == USB_HIGH) {
		...
		sq_usb_set_max_pktsize(USB_HIGH);
	}
	else if(((status&0x6) >>1) == USB_FULL) {
		DBG_SETUP1("Full Speed Detec tion\n");
		sq_usb_set_max_pktsize(USB_FULL);
	}
	else {
	...
	sq_usb_set_endpoint();
	sq_usb_set_descriptors();
	sq_usb_clear_dnfile_info();
	sq_usb_set_opmode(op_mode);

	return TRUE;
}

void sq_usb_pkt_receive(void)
{
	u32 rx_status;
	u32 fifo_cnt_byte;

	rx_status = readl(SQ_OTG_GRXSTSP);
	DBG_SETUP0("SQ_OTG_GRXSTSP = 0x%x\n", rx_status);

	if ((rx_status & (0xf<<17)) == SETUP_PKT_RECEIVED) {
		DBG_SETUP1("SETUP_PKT_RECEIVED\n");
		sq_usb_ep0_int_hndlr();
		...
		if((rx_status & BULK_OUT_EP)&&(fifo_cnt_byte)) {
			sq_usb_int_bulkout(fifo_cnt_byte);
			if( otg.op_mode == USB_CPU )
...
void sq_usb_transfer(void)
{
	u32 ep_int;
	u32 check_dma;
	u32 ep_int_status;

	ep_int = readl(SQ_OTG_DAINT);
	DBG_SETUP0("SQ_OTG_DAINT = 0x%x", ep_int);

	if (ep_int & (1<<CONTROL_EP)) {
		ep_int_status = readl(SQ_OTG_DIEPINT0);
		DBG_SETUP1("SQ_OTG_DIEPINT0 : %x \n", ep_int_status);

		if (ep_int_status & INTKN_TXFEMP) {
			u32 uNTxFifoSpace;
			do {
				uNTxFifoSpace=readl(SQ_OTG_GNPTXSTS)&0xffff;
			}while(uNTxFifoSpace<otg.ctrl_max_pktsize);

			sq_usb_transfer_ep0();
		}

		writel(ep_int_status, SQ_OTG_DIEPINT0); /* Interrupt Clear */
	}

	if (ep_int & ((1<<CONTROL_EP)<<16)) {
		ep_int_status = readl(SQ_OTG_DOEPINT0);
		DBG_SETUP1("SQ_OTG_DOEPINT0 : %x \n", ep_int_status);

		sq_usb_set_outep_xfersize(EP_TYPE_CONTROL, 1, 8);
		writel(1u<<31|1<<26, SQ_OTG_DOEPCTL0); /*ep0 enable, clear nak */

		writel(ep_int_status, SQ_OTG_DOEPINT0); /* Interrupt Clear */
	}

	if(ep_int & (1<<BULK_IN_EP)) {
		ep_int_status = readl(SQ_OTG_DIEPINT_IN);
		DBG_BULK1("SQ_OTG_DIEPINT_IN : %x \n", ep_int_status);
		writel(ep_int_status, SQ_OTG_DIEPINT_IN); /* Interrupt Clear */

		if ( (ep_int_status&INTKN_TXFEMP) && otg.op_mode == USB_CPU)
			sq_usb_int_bulkin();

		check_dma = readl(SQ_OTG_GAHBCFG);
		if ((check_dma&MODE_DMA)&&(ep_int_status&TRANSFER_DONE))
			sq_usb_dma_in_done();
	}

	if (ep_int & ((1<<BULK_OUT_EP)<<16)) {
		ep_int_status = readl(SQ_OTG_DOEPINT_OUT);
		DBG_BULK1("SQ_OTG_DOEPINT_OUT : 0x%x\n", ep_int_status);
		writel(ep_int_status, SQ_OTG_DOEPINT_OUT); /* Interrupt Clear */
...
void sq_udc_int_hndlr(void)
{
	u32 int_status;
	int tmp;

	int_status = readl(SQ_OTG_GINTSTS); /* Core Interrupt Register */
	writel(int_status, SQ_OTG_GINTSTS); /* Interrupt Clear */
	DBG_SETUP0("*** USB OTG Interrupt(SQ_OTG_GINTSTS: 0x%08x) ****\n",
		int_status);

	if (int_status & INT_RESET) {
		DBG_SETUP1("INT_RESET\n");
		writel(INT_RESET, SQ_OTG_GINTSTS); /* Interrupt Clear */
	...
	if (int_status & INT_ENUMDONE) {
		...
		writel(INT_ENUMDONE, SQ_OTG_GINTSTS); /* Interrupt Clear */
	...
	if (int_status & INT_RESUME) {
		...
		writel(INT_RESUME, SQ_OTG_GINTSTS); /* Interrupt Clear */
	...
	if (int_status & INT_SUSPEND) {
		...
		writel(INT_SUSPEND, SQ_OTG_GINTSTS); /* Interrupt Clear */
