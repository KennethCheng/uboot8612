...
#define RELEASE_MARK "2010-11"

#if defined(SQ_BOOT_MEDIA_NAND)
#define BOOT_MEDIA "NAND"
#elif defined(SQ_BOOT_MEDIA_SD)
...
void SQMenu(void)
{
	while(1) {
		int c;
		printf("##### SQ U-Boot(" RELEASE_MARK ", " BOOT_MEDIA ") for SQ8612 #####\n");
		...
		printf("NAND: %u MiB, RAM: %u MiB\n", SQGetNandSizeInMB(), PHYS_SDRAM_1_SIZE >> 20);
		if (Lcd != 0) {
		...
		case 'V': case 'v':
			SQGetDataFromUsbAndWriteNand(256 K, 0, 256 K, "U-Boot.bin");
			break;

		case 'K': case 'k':
			SQGetDataFromUsbAndWriteNand(5 M - 128 K, 4 * 128 K, 5 M, "Linux/Android Kernel");
			break;

		case 'Y': case 'y':
			SQGetDataFromUsbAndWriteNand(126 M, 5 M + 4 * 128 K, (unsigned)-1, "yaffs2-image");
			break;

		case 'A': case 'a':
			SQGetDataFromUsbAndWriteNand(64 M, 0, 128 M, "User-Bin");
			break;

		case 'N': case 'n':
			SQGetDataFromUsbAndWriteNand(128 K, 0, 128 K, "nboot.nb0");
			break;

		case 'W': case 'w':
			ExecuteCmd("nand erase 4280000");
			SQGetDataFromUsbAndWriteNand(63 M, 2 M + 4 * 128 K, 64 M, "NK.nb0");
			// Mark the indicators of NK Magic Number and Image Size
			{
				...
				((unsigned *)p)[1] = 63 M;
				SQWriteNand(p, 128 K, 2 * 128 K, 128 K);
			}
			break;
...
extern int SQGetDataFromUSB (unsigned max_len, unsigned char **data_ptr, unsigned *received_len);
extern int SQWriteNand(const unsigned char*data, unsigned len, unsigned offset, unsigned MaxNandSize);
int SQGetDataFromUsbAndWriteNand(unsigned max_len, unsigned offset, unsigned MaxNandSize, const char *Name)
{
	int ret;
	unsigned char *RevPtr;
	unsigned RevLen;
	printf("Downloading %s from USB...\n", Name);
	ret = SQGetDataFromUSB(max_len, &RevPtr, &RevLen);
	printf("Downloading %s %s\n", Name, ret >= 0 ? "successed" : "failed");
	if (ret < 0) {
		return ret;
	}
	printf("Writing %s into NAND...\n", Name);
	ret = SQWriteNand(RevPtr, RevLen, offset, MaxNandSize);
	printf("Writing %s %s\n", Name, ret >= 0 ? "successed" : "failed");
